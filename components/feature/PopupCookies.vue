<template>
  <Transition name="slide-right">
    <div v-if="isVisible" class="fixed bottom-0 right-0 md:bottom-10 md:right-10 p-5 md:p-0 z-50">
      <div
        ...
        <p class="text-base text-white">
          {{ props.description }}
          <nuxt-link class="text-gold transition duration-200 underline" :to="props.privacyPolicyLink">{{ props.privacyPolicyText }}
          </nuxt-link>
        </p>
        ...
        <div class="mt-3 flex flex-nowrap justify-end md:items-center">
          <button
            ...
            <svg v-if="settingsExpanded" class="inline-block w-4 h-4 -rotate-90" xmlns="http://www.w3.org/2000/svg"
            ...
            <span v-else class="inline-block text-sm">
              {{ props.settingsText }}
            </span>
          </button>
          <button v-if="settingsExpanded"
            class="md:w-max grow-0 transition duration-200 text-sm text-gray-light font-light py-0.5 px-1 md:p-2 hover:bg-gold/5"
            @click="setConsent()">
            {{ props.selectedConsentText }}
          </button>
          <UiButton class="[&_button]:!text-base [&_button]:!py-2 [&_button]:!px-5 ml-2 md:ml-3"
            :text="props.allConsentText" @click.native="setConsentAll()" />
        </div>
      </div>
    </div>
  </Transition>
...
const props = defineProps({
  description: {
    type: String,
    default: 'Ta strona używa plików cookies, aby dostosować treści i analizować ruch na stronie.',
  },
  privacyPolicyText: {
    type: String,
...
// Visibility logic
const isVisible = computed(() => isMounted.value && !(gaCookie.value || consentCookie.value));
